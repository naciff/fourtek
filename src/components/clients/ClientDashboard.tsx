...
function SortableWidget({ id, children, className = "" }: { id: string, children: React.ReactNode, className?: string }) {
    ...
    return (
        <div ref={setNodeRef} style={style} className={`h-full ${className}`}>
            {/* The whole card container acts as draggable by default with attributes/listeners */}
            {/* If we want only header to drag, we'd pass attributes/listeners to a handle inside children */}
...
export function ClientDashboard({ clientId }: { clientId: string }) {
    ...
    useEffect(() => {
        // Visibility
        const savedVisibility = localStorage.getItem('client_dashboard_widgets');
        if (savedVisibility) {
            try {
        ...
        // Order
        const savedOrder = localStorage.getItem('client_dashboard_order');
        if (savedOrder) {
            try {
                const parsedOrder = JSON.parse(savedOrder);
                // Ensure all widgets exist in the saved order (in case new ones were added)
                const mergedOrder = parsedOrder.filter((id: string) => WIDGETS.some(w => w.id === id));
                const newWidgets = WIDGETS.filter(w => !mergedOrder.includes(w.id)).map(w => w.id);
                setWidgetOrder([...mergedOrder, ...newWidgets]);
            } catch (e) { console.error(e); }
        }
    }, []);
