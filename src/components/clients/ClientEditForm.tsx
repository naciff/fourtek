...
export function ClientEditForm({ clientId }: { clientId?: string }) {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    // Sistemas
    if (isNew) {
      if (selectedSistemas.length) await supabase.from("client_sistemas").insert(selectedSistemas.map((sid) => ({ client_id: newId, sistema_id: sid })));
    } else {
      const sisBefore = await supabase.from("client_sistemas").select("sistema_id").eq("client_id", id);
      const existingSis = (sisBefore.data ?? []).map((x: any) => x.sistema_id);
      const addSis = selectedSistemas.filter((sid) => !existingSis.includes(sid));
      const removeSis = existingSis.filter((sid) => !selectedSistemas.includes(sid));
      if (addSis.length) await supabase.from("client_sistemas").insert(addSis.map((sid) => ({ client_id: id, sistema_id: sid })));
      if (removeSis.length) await supabase.from("client_sistemas").delete().in("sistema_id", removeSis).eq("client_id", id);
    }
  ...
  if (!form) return <div className="p-8 text-center text-gray-500 dark:text-gray-400">Carregando cliente...</div>;
  ...
  const navTabClass = (t: string) => `flex items-center gap-2 px-4 py-2 rounded-md border text-sm font-medium transition-colors whitespace-nowrap ${tab === t ? 'bg-brand-green-600 text-white border-brand-green-600 shadow-sm' : 'bg-green-50 text-brand-green-700 border-green-200 hover:bg-green-100 dark:bg-gray-800 dark:text-gray-300 dark:border-gray-700 dark:hover:bg-gray-700'}`;
  const subTabClass = (t: string) => `px-4 py-2 border-b-2 whitespace-nowrap transition-colors ${activeTab === t ? 'border-brand-blue-600 text-brand-blue-600 font-medium' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-200'}`;
  ...
  return (
    <div className="w-full p-2 md:p-4 space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{isNew ? 'Novo Cliente' : 'Editar Cliente'}</h1>
...

      {
        tab === 'inventario' && (
          <div className="space-y-6 animate-in fade-in">
            ...
            {showInvForm && (
              <div className="rounded-lg border bg-white dark:bg-gray-800 dark:border-gray-700 p-6 shadow-sm">
                <h3 className="text-base font-medium text-gray-900 dark:text-white mb-4">{editingInvId ? "Editar Item" : "Novo Item"}</h3>
              ...
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-gray-700 dark:text-gray-200">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Qtd</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                  {invHist.length === 0 ? <tr><td colSpan={5} className="px-6 py-4 text-center text-sm text-gray-500 dark:text-gray-400">Nenhum item registrado.</td></tr> :
                    invHist.map(item => (
                      <tr key={item.id}>
                        <td className="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">{item.item}</td>
                        <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{item.qtd}</td>
                        <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{item.descricao}</td>
                        <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{new Date(item.created_at).toLocaleDateString()}</td>
...

      {
        tab === 'acesso' && (
          <div className="space-y-6 animate-in fade-in">
            ...
            {showAcessoForm && (
              <div className="rounded-lg border bg-white dark:bg-gray-800 dark:border-gray-700 p-6 shadow-sm">
                <h3 className="text-base font-medium text-gray-900 dark:text-white mb-4">{editingAcessoId ? "Editar Dado de Acesso" : "Novo Dado de Acesso"}</h3>
              ...
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-gray-700 dark:text-gray-200">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Acesso / Link</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Credenciais</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                  {acessos.length === 0 ? <tr><td colSpan={5} className="px-6 py-4 text-center text-sm text-gray-500 dark:text-gray-400">Nenhum acesso registrado.</td></tr> :
                    acessos.map(acesso => (
                      <tr key={acesso.id}>
                        <td className="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">{acesso.tipo_acesso}</td>
...

      {
        tab === 'servidores' && (
          <div className="space-y-6 animate-in fade-in">
            ...
            {showServerForm && (
              <div className="rounded-lg border bg-white dark:bg-gray-800 dark:border-gray-700 p-6 shadow-sm">
                <h3 className="text-base font-medium text-gray-900 dark:text-white mb-4">{editingServerId ? "Editar Servidor" : "Novo Servidor"}</h3>
            ...
            <div className="space-y-8">
              {[
                { title: 'Firewall', color: 'text-red-600 dark:text-red-400', filter: (s: any) => s.hostname === 'Corvette (Firewall)' },
                {
                  title: 'Servidores',
                  color: 'text-brand-blue-600 dark:text-blue-400',
                ...
                { title: 'Storage', color: 'text-gray-700 dark:text-gray-300', filter: (s: any) => s.hostname === 'Storage' },
                { title: 'Outros', color: 'text-gray-700 dark:text-gray-300', filter: (s: any) => !['BMW', 'Ferrari', 'Porsche', 'Storage', 'Corvette (Firewall)'].includes(s.hostname) }
              ].map((group) => {
                ...
                return (
                  <div key={group.title} className="rounded-lg border bg-white dark:bg-gray-800 dark:border-gray-700 overflow-hidden shadow-sm animate-in fade-in slide-in-from-bottom-2 duration-500">
                    <div className="px-6 py-3 bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600 flex items-center gap-2">
                      <h4 className={`text-sm font-bold uppercase tracking-wide ${group.color}`}>{group.title}</h4>
                      <span className="bg-gray-200 dark:bg-gray-600 text-gray-600 dark:text-gray-200 py-0.5 px-2 rounded-full text-xs font-medium">{groupServers.length}</span>
                    </div>
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                      <thead className="bg-white text-gray-500 dark:text-gray-400">
                        <tr>
                        ...
                        {groupServers.map(s => {
                          ...
                          return (
                            <React.Fragment key={s.id}>
                              <tr className={`hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors ${expandedServers.has(s.id) ? "bg-blue-50/30 dark:bg-blue-900/20" : ""}`}>
                                ...
                                <td className="px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white">{s.hostname}</td>
                                <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{s.server_brand || s.storage_brand || '-'}</td>
                                <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
                                  <button
                                ...
                                <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{s.os}</td>
                                <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
                              ...
                              {expandedServers.has(s.id) && (
                                <tr className="bg-gray-50/50 dark:bg-gray-900/50 shadow-inner">
                                  <td colSpan={7} className="px-0 py-0 border-t border-gray-100 dark:border-gray-700">
                                    <div className="p-4 pl-16 pr-8 bg-slate-50 dark:bg-gray-800 border-l-4 border-brand-blue-500">
                                          ...
                                          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                            <thead className="bg-gray-50 dark:bg-gray-700 dark:text-gray-200">
                                              <tr>
                                                ...
                                                <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Credenciais</th>
                                            ...
                                            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                                              {vms.filter(v => v.server_id === s.id).map(vm => (
                                                <tr key={vm.id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                                                  ...
                                                  <td className="px-4 py-2 text-sm text-gray-500 dark:text-gray-400">{vm.os}</td>
              ...
              {servers.length === 0 && (
                <div className="rounded-lg border bg-white p-8 text-center text-gray-500 dark:text-gray-400">
...

      {
        tab === 'sistemas' && (
          <div className="space-y-6 animate-in fade-in">
            <h3 className="text-lg font-medium text-gray-900">Sistemas Contratados</h3>
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border dark:border-gray-700 shadow-sm">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {sistemasList.map((sis) => (
                  <label key={sis.id} className="flex items-center space-x-3 p-3 rounded-lg border hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer transition-colors has-[:checked]:border-brand-green-500 has-[:checked]:bg-brand-green-50 dark:has-[:checked]:bg-brand-green-900/20">
                    <input
                      type="checkbox"
                      checked={selectedSistemas.includes(sis.id)}
                      onChange={(e) => {
                        if (e.target.checked) setSelectedSistemas([...selectedSistemas, sis.id]);
                        else setSelectedSistemas(selectedSistemas.filter((id) => id !== sis.id));
                      }}
                      className="h-5 w-5 rounded border-gray-300 text-brand-green-600 focus:ring-brand-green-600"
                    />
                    <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{sis.name}</span>
                  </label>
                ))}
              </div>

              {sistemasList.length === 0 && (
                <p className="text-gray-500 text-sm italic">Nenhum sistema cadastrado.</p>
              )}
            </div>

            <div className="bg-gray-50 dark:bg-gray-800 p-6 rounded-lg border dark:border-gray-700">
              <h4 className="text-base font-medium text-gray-900 dark:text-white mb-4">Cadastrar Novo Sistema</h4>
              <div className="flex gap-2 max-w-md">
                ...
                <button
                  ...
                  onClick={async () => {
                    if (!newSistemaName.trim()) return;
                    setLoading(true);
                    const slug = newSistemaName.trim().toLowerCase().replace(/\s+/g, '-');
                    const { data, error } = await supabase.from('sistemas').insert({ name: newSistemaName.trim(), slug }).select().single();
                    if (data) {
                      setSistemasList((prev) => [...prev, data].sort((a, b) => a.name.localeCompare(b.name)));
                      setNewSistemaName("");
                  ...
                  disabled={loading || !newSistemaName.trim()}
      ...
      {tab === 'prov74' && (
        <div className="space-y-8 animate-in fade-in">
          ...
          {/* Form */}
          {showProv74Form && (
            <div className="rounded-lg border bg-white dark:bg-gray-800 dark:border-gray-700 p-6 shadow-sm border-l-4 border-l-brand-blue-500 animate-in slide-in-from-top-2">
          ...
          <div className="rounded-lg border bg-white dark:bg-gray-800 dark:border-gray-700 overflow-hidden shadow-sm">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-50 dark:bg-gray-700 dark:text-gray-200">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Situação</th>
                  <th className="px-6 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">DataHora</th>
                  <th className="px-6 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Itens Verificados</th>
                  <th className="px-6 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Observação</th>
                  <th className="px-6 py-3 text-right text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Ações</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                {prov74History.length === 0 ? (
                  <tr><td colSpan={5} className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">Nenhum item</td></tr>
                ) : (
                  prov74History.map(h => (
                    <tr key={h.id}>
                      ...
                      <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{new Date(h.data_hora).toLocaleString('pt-BR')}</td>
                      <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
      ...
      {tab === 'relatorios' && (
        <div className="space-y-6 animate-in fade-in">
              ...
              <form onSubmit={onRelatoriosSubmit}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  ...
                  <div className="flex flex-col gap-1">
                    <span className="text-xs text-gray-500 dark:text-gray-400">Arquivo</span>
          ...
          <div className="rounded-lg border bg-white dark:bg-gray-800 dark:border-gray-700 overflow-hidden shadow-sm">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-green-50 dark:bg-gray-700 dark:text-gray-200">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Tipo</th>
                  <th className="px-6 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Data</th>
                  <th className="px-6 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Versão</th>
                  <th className="px-6 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Arquivo</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                {relatoriosHistory.length === 0 ? (
                  <tr><td colSpan={4} className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">Nenhum relatório</td></tr>
                ) : (
                  relatoriosHistory.map(h => (
                    <tr key={h.id}>
                      <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">{h.type}</td>
                      <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{h.report_date ? new Date(h.report_date).toLocaleDateString() : '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{h.version}</td>
      ...
      {tab === 'pcn' && (
        <div className="space-y-6 animate-in fade-in">
          ...
          <div className="rounded-lg border bg-white dark:bg-gray-800 dark:border-gray-700 overflow-hidden shadow-sm">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-blue-50 dark:bg-gray-700 dark:text-gray-200">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">PCN</th>
                  <th className="px-6 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">PoliticaBackup</th>
                  <th className="px-6 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">PoliticaTI</th>
                  <th className="px-6 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Encaminhado</th>
                  <th className="px-6 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Link</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                {pcnHistory.length === 0 ? (
                  <tr><td colSpan={5} className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">Nenhum registro</td></tr>
                ) : (
                  pcnHistory.map(h => (
                    <tr key={h.id}>
                      <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">{h.pcn ? 'Sim' : 'Não'}</td>
                      <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">{h.politica_backup ? 'Sim' : 'Não'}</td>
                      <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">{h.politica_ti ? 'Sim' : 'Não'}</td>
                      <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">{h.encaminhado ? 'Sim' : 'Não'}</td>
          ...
          <div className="rounded-lg border bg-white dark:bg-gray-800 dark:border-gray-700 overflow-hidden shadow-sm">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-green-50 dark:bg-gray-700 dark:text-gray-200">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Tipo</th>
                  <th className="px-6 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Qtd</th>
                  <th className="px-6 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Marca/Modelo</th>
                  <th className="px-6 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Ações</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                {dadosAdicionaisHistory.length === 0 ? (
                  <tr><td colSpan={4} className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">Nenhum registro</td></tr>
                ) : (
                  dadosAdicionaisHistory.map(h => (
                    <tr key={h.id}>
                      <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">{h.type}</td>
                      <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">{h.quantity}</td>
                      <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">{h.brand_model || '-'}</td>
