...
async function tryInsertSupabase(user: any) {
  try {
    ...
    const { data, error } = await supabase.from("users").insert({
      user_id: user.user_id || uid, // Use provided user_id or current user
...
async function tryListSupabase() {
  try {
    ...
    const { data, error } = await supabase.from("users")
      .select("id,full_name,email,group,permissions,last_login,phone,last_seen_at,created_at")
...
export async function POST(req: Request) {
  try {
    ...
    // 2. Create metadata in users table
    ...
    const user = { id: Date.now(), full_name, email, group: group || "gestor", permissions: permissions || [], password_hash, user_id: authData.user.id };
...
export async function PATCH(req: Request) {
  try {
    ...
    console.log('Updating user:', id, 'with data:', update);
